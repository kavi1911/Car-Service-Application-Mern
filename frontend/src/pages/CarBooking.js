...
const CarBooking = ({ match }) => {
  ...
  const [totalAmount, setTotalAmount] = useState(car.capacity);
  ...
  return (
    <DefaultLayout>
      <h5 style={{margin:'20px', textAlign:'left'}}><Link to="/">&larr; back</Link></h5>
      <div className="row car">
        ...
        >
          ...
          <div>
            <p>{car.name}</p>
            <p>Time-Taken {car.rentPerHour} Hrs</p>
            <p>Price Rs : {car.capacity}</p>
            <p>Description : {car.feulType}</p>
          </div>
          {/* <Divider type="horizontal" dahsed>
            SELECT TIME SLOT
          </Divider>
